...
function Column({ disabled, item }: any) {
	enum types {
		STRING = "STRING",
		NUMBER = "NUMBER",
		TIME = "TIME",
	...
	const [type, setType] = useState(item?.type || types.STRING);
	return (
		<>
			<div>
				<label
...
export default function Comp({ searchParams }: any) {
	...
	useEffect(() => {
		if (data) {
			setCols(data.item.columns.length);
		}
		if (state.message != "") {
			toast({
